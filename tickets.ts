...
export function calculateOptimalTicket(selectedDates: Date[]) {
  ...
  const datesByMonth: DatesByMonth = selectedDates.reduce(
    (acc: DatesByMonth, date) => {
  ...
  if (Object.keys(datesByMonth).length > 1) {
    ...
    const monthlyPassSpanTwoMonths =
      monthlyPassPossible && firstMonthNonEmpty && secondMonthNonEmpty;
